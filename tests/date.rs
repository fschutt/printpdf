...
fn test_date_serialization() {
    let date = Date { year: 2025, month: 3, day: 19 };
...
fn test_time_serialization() {
    let time = Time { hour: 14, minute: 30, second: 45, millisecond: 123 };
...
fn test_offset_serialization() {
    let offset = Offset { hours: 2, minutes: 30, seconds: 0, milliseconds: 0 };
    let serialized = serde_json::to_string(&offset).unwrap();
    assert_eq!(serialized, "\"+02:30:00.000\"");

    let negative_offset = Offset { hours: -5, minutes: -30, seconds: 0, milliseconds: 0 };
...
fn test_datetime_serialization() {
    let dt = DateTime {
        date: Date { year: 2025, month: 3, day: 19 },
        time: Time { hour: 14, minute: 30, second: 45, millisecond: 123 },
        offset: Offset { hours: 2, minutes: 30, seconds: 0, milliseconds: 0 },
    };
...
fn test_edge_cases() {
    // Leap year
    let leap_date = Date { year: 2024, month: 2, day: 29 };
    ...
    let dt = DateTime {
        ...
        time: Time { hour: 23, minute: 59, second: 59, millisecond: 999 },
