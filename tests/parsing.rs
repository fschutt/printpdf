...
use printpdf::{
    BlendMode, BuiltinFont, Color, ExtendedGraphicsState, FontId, BuiltinOrExternalFontId,
    Layer, LayerIntent, LayerInternalId, LayerSubtype, LineCapStyle, LineDashPattern, LineJoinStyle, 
    Mm, Op, OverprintMode, PageAnnotId, PdfDocument, PdfPage, PdfParseOptions, PdfSaveOptions, 
    PolygonRing, Point, Polygon, Pt, RenderingIntent, Rgb, SeperableBlendMode, TextItem, TextRenderingMode
};
...
fn test_layer_parsing() {
    ...
    let ops = vec![
        ...
        Op::EndLayer { layer_id: layer_id.clone() },
    ];
    ...
    assert!(!parsed_doc.resources.layers.map.is_empty(), "No layers found in parsed document");
...
fn test_extgstate_parsing() {
    ...
    assert!(!parsed_doc.resources.extgstates.map.is_empty(), "No extended graphics states found in parsed document");
...
fn test_bookmark_parsing() {
    ...
    let bookmarks: Vec<(&PageAnnotId, &printpdf::PageAnnotation)> = parsed_doc.bookmarks.map.iter().collect();
    
    let chapter1 = bookmarks.iter().find(|(_, b)| b.name == "Chapter 1").unwrap().1;
    let chapter2 = bookmarks.iter().find(|(_, b)| b.name == "Chapter 2").unwrap().1;
    let chapter3 = bookmarks.iter().find(|(_, b)| b.name == "Chapter 3").unwrap().1;
...
fn test_complex_document_parsing() {
    ...
    let ops1 = vec![
        Op::BeginLayer { layer_id: layer1_id.clone() },
        Op::SaveGraphicsState,
        ...
        Op::EndTextSection,
        Op::RestoreGraphicsState,
        Op::EndLayer { layer_id: layer1_id.clone() },
    ];
    
    // Page 2: Layer 2 with GS 2
    let ops2 = vec![
        Op::BeginLayer { layer_id: layer2_id.clone() },
        Op::SaveGraphicsState,
        ...
        Op::EndTextSection,
        Op::RestoreGraphicsState,
        Op::EndLayer { layer_id: layer2_id.clone() },
    ];
    ...
    assert_eq!(parsed_doc.resources.layers.map.len(), 2, "Expected 2 layers, found {}", parsed_doc.resources.layers.map.len());
    assert_eq!(parsed_doc.resources.extgstates.map.len(), 2, "Expected 2 graphics states, found {}", parsed_doc.resources.extgstates.map.len());
    assert_eq!(parsed_doc.bookmarks.map.len(), 3, "Expected 3 bookmarks, found {}", parsed_doc.bookmarks.map.len());
    
    // Check layer names
    let layer_names: Vec<String> = parsed_doc.resources.layers.map.values()
        .map(|l| l.name.clone())
        .collect();
    ...
    let bookmark_names: Vec<String> = parsed_doc.bookmarks.map.values()
        .map(|b| b.name.clone())
        .collect();
    ...
    for (i, page) in parsed_doc.pages.iter().enumerate() {
        ...
        let layer_begins = page.ops.iter().filter(|op| {
            if let Op::BeginLayer { layer_id: _ } = op {
                true
            } else {
                false
        ...
        let layer_ends = page.ops.iter().filter(|op| {
            if let Op::EndLayer { layer_id: _ } = op {
                true
            } else {
                false
        ...
        match page_number {
            1 | 2 => {
                assert_eq!(layer_begins, 1, "Page {} should have 1 BeginLayer op", page_number);
                assert_eq!(layer_ends, 1, "Page {} should have 1 EndLayer op", page_number);
            },
            3 => {
                assert_eq!(layer_begins, 2, "Page {} should have 2 BeginLayer ops", page_number);
                assert_eq!(layer_ends, 2, "Page {} should have 2 EndLayer ops", page_number);
            },
            _ => {}
        }
        
        // Check for graphics state operations
        let gs_loads = page.ops.iter().filter(|op| {
            if let Op::LoadGraphicsState { gs: _ } = op {
                true
            } else {
                false
        ...
        match page_number {
            1 | 2 => {
                assert_eq!(gs_loads, 1, "Page {} should have 1 LoadGraphicsState op", page_number);
            },
            3 => {
                assert_eq!(gs_loads, 2, "Page {} should have 2 LoadGraphicsState ops", page_number);
            },
            _ => {}
        }
    }
...
fn test_extgstate_font_parsing() {
    ...
    assert!(!parsed_doc.resources.extgstates.map.is_empty(), "No extended graphics states found");
    
    // Verify the operations on the page
    let parsed_page = &parsed_doc.pages[0];
    
    // Check for text operations that use the font
    let has_font_ops = parsed_page.ops.iter().any(|op| {
        match op {
